__d(
  "WAWebCallLinkJoinCallLiteData",
  ["fbt"],
  function (t, n, r, o, a, i, l, s) {
    "use strict";
    var e = "0 0 1487.13 346",
      u =
        "M1127.34,188.81l21.43-65.42,21.65,65.42h-43.08ZM1428.25,229.87c-15.56,0-24.36-10.83-24.36-32.93v-5.64c0-20.08,9.25-32.26,25.26-32.26,13.08,0,23.68,9.25,23.68,34.96s-9.7,35.87-24.59,35.87ZM1370.28,297.09h33.38v-55.94c8.12,10.83,20.08,15.79,32.93,15.79,31.13,0,50.53-24.36,50.53-63.84s-16.69-61.13-48.27-61.13c-15.56,0-27.29,5.64-36.09,17.37v-13.76h-32.48v161.51ZM1296.75,229.87c-15.56,0-24.36-10.83-24.36-32.93v-5.64c0-20.08,9.25-32.26,25.26-32.26,13.08,0,23.68,9.25,23.68,34.96s-9.7,35.87-24.59,35.87ZM1238.77,297.09h33.38v-55.94c8.12,10.83,20.08,15.79,32.93,15.79,31.13,0,50.53-24.36,50.53-63.84s-16.69-61.13-48.27-61.13c-15.56,0-27.29,5.64-36.09,17.37v-13.76h-32.48v161.51ZM1070.95,253.32h35.64l11.96-35.64h60.68l11.96,35.64h36.77l-57.52-161.06h-41.73l-57.75,161.06ZM1014.33,256.93c34.06,0,51.43-13.99,51.43-36.99s-10.83-32.48-41.96-37.67l-14.66-2.48c-12.63-2.03-16.47-6.09-16.47-12.63s4.96-11.73,19.17-11.73c13.31,0,18.72,4.96,20.98,17.59h31.13c-1.8-26.84-18.95-41.05-52.11-41.05-30.9,0-50.75,13.31-50.75,36.09s13.31,31.58,43.08,36.54l13.53,2.26c12.41,2.03,15.34,6.54,15.34,13.31,0,7.89-5.19,12.86-19.4,12.86s-22.33-5.41-23.68-17.82h-31.81c1.13,29.32,22.56,41.73,56.17,41.73ZM921.85,256.03c10.38,0,20.98-2.93,27.29-7.22v-25.26c-6.32,3.61-12.41,5.41-18.05,5.41-9.7,0-14.89-4.06-14.89-16.47v-50.75h32.93v-26.17h-32.93v-32.48h-29.78v18.05c0,10.15-2.48,14.44-12.63,14.44h-8.8v26.17h17.82v51.88c0,27.29,10.38,42.41,39.02,42.41ZM796.89,233.47c-10.83,0-16.92-4.74-16.92-13.08,0-9.47,6.99-13.99,23.23-16.69,9.02-1.58,16.02-3.38,21.43-7.22v11.5c0,15.56-11.05,25.49-27.75,25.49ZM788.31,256.93c17.37,0,29.55-7.22,39.02-17.82,1.13,5.64,3.16,10.38,5.86,14.21h31.58c-5.64-8.8-8.12-21.88-8.12-39.25v-37.67c0-27.97-14.89-44.44-50.08-44.44-31.13,0-49.4,12.86-53.01,41.05h30.45c1.8-10.83,8.35-17.14,21.43-17.14,12.18,0,19.17,4.96,19.17,13.53s-5.19,11.5-28.2,14.89c-25.04,3.61-48.05,12.63-48.05,37.9,0,22.56,16.02,34.74,39.93,34.74ZM625.23,253.32h33.38v-69.02c0-8.12,2.03-11.96,6.99-16.92,4.96-4.96,11.5-7.67,18.5-7.67,11.05,0,16.92,6.09,16.92,20.98v72.63h33.38v-78.95c0-27.29-13.53-42.41-39.02-42.41-13.08,0-25.26,4.06-36.77,17.59v-57.29h-33.38v161.06ZM444.09,253.32h36.77l26.62-116.17,27.07,116.17h37.22l42.63-161.06h-36.99l-25.26,118.87-27.07-117.97h-33.38l-27.52,118.42-25.26-119.33h-38.12l43.31,161.06Z",
      c =
        "M173,0C77.45,0,0,77.45,0,173c0,31.43,8.38,60.91,23.04,86.31L0,346l89.87-21.25c24.67,13.54,53,21.25,83.13,21.25,95.55,0,173-77.45,173-173S268.55,0,173,0ZM173,315.01c-28.91,0-55.81-8.64-78.24-23.48l-53.1,13.52,14.89-50.75c-16.11-23.03-25.56-51.06-25.56-81.3,0-78.43,63.58-142.01,142.01-142.01s142.01,63.58,142.01,142.01-63.58,142.01-142.01,142.01Z",
      d =
        "M213.54,195.84l41.86,19.73c1.92.91,3.15,2.85,2.98,4.97-.45,5.51-2.66,16.55-12.56,26.44-27.93,27.93-78.09-3.67-80.13-4.89-12.34-6.63-24.06-15.49-35.17-26.61-11.11-11.11-19.98-22.84-26.61-35.17-1.22-2.04-32.82-52.19-4.89-80.13,9.9-9.9,20.93-12.1,26.44-12.56,2.12-.17,4.07,1.06,4.97,2.98l19.73,41.86c.93,1.98.52,4.33-1.02,5.88l-14.71,14.71c-3.18,3.18-4.12,8.13-1.92,12.06,5.37,9.63,12.59,18.9,20.95,27.43,8.53,8.36,17.8,15.58,27.43,20.95,3.93,2.19,8.88,1.26,12.06-1.92l14.71-14.71c1.55-1.55,3.9-1.96,5.88-1.02Z",
      m = function (n) {
        var t =
          "<svg xmlns='http://www.w3.org/2000/svg' viewBox='" +
          e +
          "' fill='" +
          n +
          "'><path d='" +
          u +
          "'/><path d='" +
          c +
          "'/><path d='" +
          d +
          "'/></svg>";
        return "data:image/svg+xml;utf8," + encodeURIComponent(t);
      },
      p = m("#25D366"),
      _ = m("#FFFFFF"),
      f = [
        {
          heading: function () {
            return s._(/*BTDS*/ "What we do");
          },
          links: [
            {
              label: function () {
                return s._(/*BTDS*/ "Features");
              },
              href: "https://www.whatsapp.com/features",
            },
            {
              label: function () {
                return s._(/*BTDS*/ "Blog");
              },
              href: "https://blog.whatsapp.com/",
            },
            {
              label: function () {
                return s._(/*BTDS*/ "Security");
              },
              href: "https://www.whatsapp.com/security",
            },
            {
              label: function () {
                return s._(/*BTDS*/ "For Business");
              },
              href: "https://business.whatsapp.com/",
            },
          ],
        },
        {
          heading: function () {
            return s._(/*BTDS*/ "Who we are");
          },
          links: [
            {
              label: function () {
                return s._(/*BTDS*/ "About us");
              },
              href: "https://www.whatsapp.com/about",
            },
            {
              label: function () {
                return s._(/*BTDS*/ "Careers");
              },
              href: "https://www.whatsapp.com/careers",
            },
            {
              label: function () {
                return s._(/*BTDS*/ "Brand Center");
              },
              href: "https://www.whatsapp.com/brand",
            },
            {
              label: function () {
                return s._(/*BTDS*/ "Privacy");
              },
              href: "https://www.whatsapp.com/privacy",
            },
          ],
        },
        {
          heading: function () {
            return s._(/*BTDS*/ "Use WhatsApp");
          },
          links: [
            {
              label: function () {
                return s._(/*BTDS*/ "Android");
              },
              href: "https://www.whatsapp.com/android",
            },
            {
              label: function () {
                return s._(/*BTDS*/ "iPhone");
              },
              href: "https://www.whatsapp.com/iphone",
            },
            {
              label: function () {
                return s._(/*BTDS*/ "Mac\/PC");
              },
              href: "https://www.whatsapp.com/download",
            },
            {
              label: function () {
                return s._(/*BTDS*/ "WhatsApp Web");
              },
              href: "https://web.whatsapp.com/",
            },
          ],
        },
        {
          heading: function () {
            return s._(/*BTDS*/ "Need help?");
          },
          links: [
            {
              label: function () {
                return s._(/*BTDS*/ "Contact Us");
              },
              href: "https://www.whatsapp.com/contact",
            },
            {
              label: function () {
                return s._(/*BTDS*/ "Help Center");
              },
              href: "https://faq.whatsapp.com/",
            },
            {
              label: function () {
                return s._(/*BTDS*/ "Apps");
              },
              href: "https://www.whatsapp.com/download",
            },
            {
              label: function () {
                return s._(/*BTDS*/ "Security Advisories");
              },
              href: "https://www.whatsapp.com/security/advisories/",
            },
          ],
        },
      ],
      g = [
        {
          title: function () {
            return s._(/*BTDS*/ "Calling");
          },
          description: function () {
            return s._(/*BTDS*/ "Stay connected with voice and video.");
          },
          href: "https://www.whatsapp.com/features/calling",
        },
        {
          title: function () {
            return s._(/*BTDS*/ "Messaging");
          },
          description: function () {
            return s._(/*BTDS*/ "Say it with stickers, voice, GIFs and more.");
          },
          href: "https://www.whatsapp.com/features/messaging",
        },
        {
          title: function () {
            return s._(/*BTDS*/ "Groups");
          },
          description: function () {
            return s._(
              /*BTDS*/ "Messaging features you need to connect in groups.",
            );
          },
          href: "https://www.whatsapp.com/features/groups",
        },
        {
          title: function () {
            return s._(/*BTDS*/ "Channels");
          },
          description: function () {
            return s._(/*BTDS*/ "Stay updated on topics you care about.");
          },
          href: "https://www.whatsapp.com/channels",
        },
        {
          title: function () {
            return s._(/*BTDS*/ "Meta AI");
          },
          description: function () {
            return s._(
              /*BTDS*/ "Get help with anything, all with the safety and privacy on WhatsApp.",
            );
          },
          href: "https://www.whatsapp.com/features/meta-ai",
        },
        {
          title: function () {
            return s._(/*BTDS*/ "Status");
          },
          description: function () {
            return s._(
              /*BTDS*/ "Share your everyday with photos, videos and voice notes.",
            );
          },
          href: "https://www.whatsapp.com/features/status",
        },
        {
          title: function () {
            return s._(/*BTDS*/ "Security");
          },
          description: function () {
            return s._(/*BTDS*/ "Layers of protection to help you stay safe.");
          },
          href: "https://www.whatsapp.com/security",
        },
      ],
      h = [
        {
          label: function () {
            return s._(/*BTDS*/ "Features");
          },
          href: "https://www.whatsapp.com/features",
          hasDropdown: !0,
        },
        {
          label: function () {
            return s._(/*BTDS*/ "Privacy");
          },
          href: "https://www.whatsapp.com/privacy",
        },
        {
          label: function () {
            return s._(/*BTDS*/ "Help Center");
          },
          href: "https://faq.whatsapp.com/",
        },
        {
          label: function () {
            return s._(/*BTDS*/ "Blog");
          },
          href: "https://blog.whatsapp.com/",
        },
        {
          label: function () {
            return s._(/*BTDS*/ "For Business");
          },
          href: "https://business.whatsapp.com/",
        },
        {
          label: function () {
            return s._(/*BTDS*/ "Apps");
          },
          href: "https://www.whatsapp.com/download",
        },
      ];
    ((l.COMBINED_LOGO_GREEN_DATA_URL = p),
      (l.COMBINED_LOGO_WHITE_DATA_URL = _),
      (l.FOOTER_COLUMNS = f),
      (l.FEATURE_DROPDOWN_ITEMS = g),
      (l.HEADER_NAV_LINKS = h));
  },
  226,
);
