__d(
  "WAWebWamEnumWsuaScreen",
  [],
  function (t, n, r, o, a, i) {
    var e = Object.freeze({
      SETTINGS: 1,
      ON_WAITLIST_BOTTOMSHEET: 2,
      JOIN_WAITLIST_BOTTOMSHEET: 3,
      QUICK_PROMOTION: 4,
      APPEARANCE_SETTINGS: 5,
      APP_ICON_SETTINGS: 6,
      APP_THEME_SETTINGS: 7,
      PROMO_BOTTOMSHEET: 8,
      DEFAULT_NOTIFICATION_SETTINGS: 9,
      DEFAULT_RINGTONE: 10,
      CHAT_NOTIFICATION_SETTINGS: 11,
      CHAT_RINGTONE: 12,
      STICKER_TRAY: 13,
      STICKER_RECEIVED: 14,
      CHAT_HOME: 15,
      LIST_SETTINGS: 16,
      LINKED_DEVICES: 17,
      CUSTOM_URL: 18,
      BUSINESS_TOOLS: 19,
      SUBSCRIPTION_MANAGEMENT: 20,
      MESSAGE_CAPPING: 21,
      PRIVACY_INTERSTITIAL: 22,
      PROTECTED_ACCOUNTS: 23,
      EDIT_PROFILE: 24,
      CALLS_TAB: 25,
      BUSINESS_CARD: 26,
      CHAT_ASSIGNMENT: 27,
      BB_CREDITS: 28,
      NOTIFICATIONS: 29,
      EDUCATION: 30,
      VALUE_SCREEN: 31,
      SUB_HUB: 32,
      CHAT_THREAD: 33,
      META_AI_SETTINGS: 34,
      VALID_NAME_BOTTOMSHEET: 35,
      CHAT_THREAD_THINKING: 36,
      CHAT_THREAD_IMAGINE: 37,
      CHAT_THREAD_VIDEO: 38,
      PROMO_ELIGIBILITY_CHECK: 39,
      TIER_SELECTION_SCREEN: 40,
      AI_TAB_THINKING: 41,
      META_ONE_ELIGIBLE_BOTTOMSHEET: 42,
      BUSINESS_BROADCAST_HOME: 43,
      BUSINESS_BROADCAST_THREAD: 44,
      BUSINESS_BROADCAST_REVIEW_SCREEN: 45,
      BIZ_AI_AGENT_THREAD: 46,
      BIZ_AI_HOME: 47,
      CHAT_LIST: 48,
      AI_HANDOFF: 49,
      SMART_COMPOSER: 50,
      CUSTOMER_LEAD_GEN: 51,
      APPT_BOOKING: 52,
      PAYMENT_CHECKOUT: 53,
      AI_RESPONSES: 54,
      IMAGINE_INTENTS_BOTTOM_SHEET: 55,
      IMAGINE_INTENTS_CREATE_IMAGE: 56,
      IMAGINE_INTENTS_EDIT_IMAGE: 57,
      IMAGINE_INTENTS_PRESETS: 58,
      IMAGINE_INTENTS_RESTYLE: 59,
      IMAGINE_INTENTS_ANIMATE: 60,
      MEDIA_EDITOR_BOTTOM_SHEET: 61,
      MEDIA_EDITOR_RESTYLE: 62,
      MEDIA_EDITOR_EDIT_OPTION: 63,
      MEDIA_EDITOR_ANIMATE: 64,
      MEDIA_EDITOR_VIDEO_RESTYLE: 65,
      IMAGINE_INTENTS_REGENERATE: 66,
      MEDIA_EDITOR_REGENERATE: 67,
      CHAT_SETTINGS: 68,
      CUSTOM_REACTIONS_SETTINGS: 69,
      REACTION_KEYBOARD: 70,
      WAPLUS_CONSUMER_WEB: 71,
      WAPLUS_CONSUMER_HOUSEAD: 72,
      META_SUBS_PRO_ACQ_WA_EMAIL: 73,
      META_SUBS_PRO_WABIZAI_EMAIL: 74,
      META_SUBS_PRO_WABIZAI_FBADS: 75,
      NCT_FIRST_WARNING: 76,
      NCT_SECOND_WARNING: 77,
      NCT_LIMIT_REACHED: 78,
      NCT_HOME: 79,
      BACKUP_SETTINGS_PROVIDER_SELECTION: 80,
      BACKUP_NUX_PROVIDER_SELECTION: 81,
      CHAT_BACKUP_BOTTOMSHEET: 82,
      SMART_COMPOSER_SELECTOR: 83,
      AI_REPLY: 84,
      META_SUBS_PRO_MSGRBIZAI_EMAIL: 85,
      META_SUBS_PRO_MSGRBIZAI_AYMT: 86,
      META_SUBS_PRO_MSGRBIZAI_FBADS: 87,
      META_SUBS_PRO_WEB: 88,
      META_SUBS_PRO_SEM_WEB: 89,
      CHAT_BACKUP_SETTINGS: 90,
      WAPLUS_CONSUMER_SEM_WEB: 91,
      CONSUMER_SUBS_PAYMENT_ABANDONMENT_NOTIF: 92,
      BIZ_SUB_VERIFICATION_ELIGIBLE_NOTIFICATION: 93,
      BIZ_SUB_VERIFICATION_ELIGIBLE_BUSINESS_ALERT: 94,
    });
    i.WSUA_SCREEN = e;
  },
  66,
);
