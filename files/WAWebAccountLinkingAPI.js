__d(
  ...
  function (t, n, r, o, a, i, l) {
    ...
    async function de(e, t, n) {
      ...
      var a = await W.getAccountLinkingData();
      if (a == null)
        throw (
          o("WALogger")
            .ERROR(
      ...
      var s = await o("WAWebAccountLinkingCryptoUtils").generateRSAKeys(),
        u = s.privateKey,
        c = s.publicKey,
        d = await o("WAWebAccountLinkingCryptoUtils").cryptoKeyToPem(c, !0),
        m = {
          version: 1,
          timestamp: Math.floor(Date.now() / 1e3),
          access_token: i,
      ...
      if (_.name === "EncryptedPayloadRequestResponseSuccess") {
        var f = o("WAWebAPIParser").parseRSAEncryptionMetadataMixin(
          ...
          b = await o(
            "WAWebAccountLinkingCryptoUtils",
          ).decryptRSAEncryptedPayload(u, h, g, y, C);
        if (
          "link_mutation_succeeded" in b &&
          b.success === !0 &&
          b.link_mutation_succeeded === !0
        )
          await W.updateAccountLinkingState(
            o("WAWebAccountLinkingConstants").AccountLinkState.Active,
          );
        else {
          var v = "success" in b ? b.success : void 0,
            S =
              "link_mutation_succeeded" in b
                ? b.link_mutation_succeeded
                : void 0;
          throw (
            o("WALogger")
              .ERROR(
        ...
        var R = _.value.errorEncryptedPayloadRequestErrors,
          L = await o("WAWebWaffleIQErrorHandler").handleCommonWaffleIQError(
            "linkAction",
            R.name,
          );
        throw (
          ...
          r("err")("Linking mutation RPC error: %s", R.name)
        );
      }
    }
